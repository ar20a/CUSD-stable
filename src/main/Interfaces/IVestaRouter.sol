...
// Interface which handles routing of tokens to between wrapped versions etc and VST or other ERC20s.
interface IVestaRouter {

    // Goes from some token (VST likely) and gives a certain amount of token out.
    ...
    // Takes the address of the token required in, and gives a certain amount of any token (VST likely) out
    ...
    // Goes from _startingTokenAddress to _endingTokenAddress, pulling tokens from _fromUser, of _amount, and gets _minSwapAmount out _endingTokenAddress. 
    // Use case: Takes token from trove debt which has been transfered to the owner and then swaps it for VST, intended to repay debt.
